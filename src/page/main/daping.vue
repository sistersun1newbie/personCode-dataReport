...
export default {
  ...
  methods: {
    ...
    start(array) {
      ...
      // 随机分 1 块钱  测试
      this.test_random({time:20000})
    },
    paixu1({array,attr=null}){
    ...
    test_Init(amound,val){
      for(let i = 0 ; i<amound ;i++){
        this.test_array.push({val:val,name:Math.random().toString(36).substring(3, 8)})

      }
    },
    test_random({amound=10,val=200,time=100}){
      let rand1,rand2;
      ...
      this.paixu1({array:this.test_array,attr:'val'})
    },
